...
class World(object):
    ...
    def restart(self):
        ...
        #mh - same car as data_collect.py
    ...
    def parse_events(self, client, world, clock):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                print("\npressed Ctrl + C or click exit button")
                # return True
...
class HUD(object):
    def __init__(self, width, height):
        ...
        # if default_font in fonts else fonts[0]
        mono = default_font
...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event):
        ...
        if self.agent is not None:
            self.agent.collision_history.append(event)
            if len(self.agent.collision_history) > 1 and \
                    event.timestamp - self.agent.collision_history[-2].timestamp < 3:
                self.agent.collision_history.pop()
...
class FrontCamera(object):
    def __init__(self, parent_actor):
        ...
        Attachment = carla.AttachmentType
        # TODO camera angle
        self.camera_transform = (carla.Transform(carla.Location(x=6.0,y=0.0,z=1.0)))
        world = self._parent.get_world()
        self.blueprint = world.get_blueprint_library().find('sensor.camera.rgb')
        self.blueprint.set_attribute('role_name', 'front_camera')  # role_name
        self.blueprint.set_attribute('sensor_tick', str(0.05))
        #switch
        #self.blueprint.set_attribute('image_size_x', str(420))
        #self.blueprint.set_attribute('image_size_y', str(280))
        
        self.blueprint.set_attribute('image_size_x', str(40))
        self.blueprint.set_attribute('image_size_y', str(40))
